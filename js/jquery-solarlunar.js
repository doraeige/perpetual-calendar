...
(function($) {
    ...
    $.fn.solarlunar = function(opts) {
        ...
        $obj.click(function () {
            ...
            obj.parent().addClass("container");
            $('.dateValue').toggleClass("input-active");
            go();
        })
        ...
        function init(option) {
            if (option.type === 'solar') {
                html = '\t<div class="solar">' +
                    ...
                    '\t\t</div>\n' +
                    '\t\t</div>\n' +
                    '\t\t<div class="time swiper-container swiper-time" id="solarTime" >\n' +
                    '\t\t<div class="swiper-wrapper">' +
                    '\t\t</div>\n' +
                    '\t\t</div>\n' +
                    '\t</div>';
                ...
                html = '\t<div class="lunar">\n' +
                    ...
                    '\t\t</div>\n' +
                    '\t\t</div>\n' +
                    '\t\t<div class="ltime swiper-container" id="lunarTime" >\n' +
                    '\t\t<div class="swiper-wrapper">' +
                    '\t\t</div>\n' +
                    '\t\t</div>\n' +
                    '\t</div>\n';
        ...
        function getNongli() {
            var nongli = solarToLunar(option.year,option.month,option.day,option.time)
        ...
        function initSolarTime(){
            ...
            for (var i = 1; i <= shichen.length; i++) {
                obj.find(".time .swiper-wrapper").append('<div class="swiper-slide" data-value="' + i + '">' + shichen[i-1] + '</div>');
            }
            let solarT = swipers("solarTime")
            solarT.on("slideChange", function () {
                option.time = obj.find(".time .swiper-slide").eq(solarT.activeIndex).data("value");
                solarTimeSw = solarT;
                setValue();
            });
            solarTimeSw =solarT;
            ...
            setActive(obj.find(".time .swiper-slide"),option.time,solarTimeSw);
            setValue();
        }
        ...
        function initLunarTime(){
            ...
            for (let i = 1; i <= shichen.length; i++) {
                obj.find(".ltime .swiper-wrapper").append('<div class="swiper-slide" data-value="' + i + '">' + shichen[i-1] + '时</div>');
            }
            var lunarT = swipers("lunarTime")
            lunarT.on("slideChange", function () {
                option.time = obj.find(".ltime .swiper-slide").eq(lunarT.activeIndex).data("value");
                lunarTimeSw = lunarT;
                setValue();
            });
            lunarTimeSw =lunarT;
            ...
            setActive(obj.find(".ltime .swiper-slide"),option.time,lunarTimeSw);
        }
        ...
        function setValue() {
            if (option.type === 'solar') {
                ...
                var shi = $(".time .swiper-slide").eq(option.time-1).text();
                var lun = solarToLunar(option.year,option.month,option.day,option.time)
                var lunar = lun.join("-");
                var solar = option.year+"-"+option.month+"-"+option.day+"-"+option.time;
                $("."+opts.textObj).val(nian + yue + ri + shi)
                    .text(nian + yue + ri + shi)
                ...
                var shi = $(".ltime .swiper-slide").eq(option.time-1).text();
                var sol = lunarToSolar(option.lyear,option.lmonth,option.lday,option.time)
                var solar = sol.join("-");
                var lunar = option.lyear + "-" + option.lmonth + "-" + option.lday + "-" + option.time;
                $("."+opts.textObj).val(nian + yue + ri + shi)
                    .text(nian + yue + ri + shi)
                    .attr("data-solar",solar)
                    .attr("data-lunar",lunar);
